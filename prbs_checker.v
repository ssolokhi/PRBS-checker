module prbs_checker #(parameter int c_LOCK_THRESHOLD = 31, parameter int c_OPEN_THRESHOLD = 31) (
    ...
    logic error = 1'b0;
    logic expected_prbs_bit = 1'b0; // define properly
    ...
    begin
        case (current_state)
            OPEN:
            begin
                if (i_received_prbs_bit != expected_prbs_bit) lock_counter <= 0; // because correct bits (if any) are not consequtive
                if (lock_counter < c_LOCK_THRESHOLD-1 && i_received_prbs_bit == expected_prbs_bit) lock_counter <= lock_counter + 1;
            ...
            LOCKED:
            begin
                if (i_received_prbs_bit == expected_prbs_bit) error <= 1'b0; 
                if (open_counter < c_OPEN_THRESHOLD-1 && i_received_prbs_bit != expected_prbs_bit) 
                begin
                    error <= 1'b1;
